<template>
  <div>
    ...
    <el-dialog title="编辑角色" :visible.sync="dialogVisible_edit" width="30%">
      <span slot="footer" class="dialog-footer">
    ...
    <el-row>
      <el-col :span="14">
        <el-card>
          ...
          <el-table :data="tableData">
            ...
            <el-table-column prop="deal" width="220px" label="操作">
              <template slot-scope="scope">
                <el-tooltip effect="dark" content="编辑" placement="top">
                  <el-button icon="el-icon-edit" size="mini" @click="editRoleShow(scope.row)" circle></el-button>
                </el-tooltip>
                <el-tooltip effect="dark" content="授权" placement="top">
                  <el-button type="success" icon="el-icon-more" size="mini" @click="editResource(scope.row)" circle></el-button>
                </el-tooltip>
                <el-tooltip effect="dark" content="分配用户" placement="top">
                  <el-button type="primary" icon="el-icon-circle-plus-outline" size="mini" @click="dispatchAdmin(scope.row.id)" circle></el-button>
                </el-tooltip>
      ...
      <el-col :offset="1" :span="8">
        <el-card>
          ...
          <el-tree ref="tree" :data="data" show-checkbox node-key="id" :default-expand-all="true" :default-checked-keys="resourceEdit.resourceList" :props="defaultProps">
          </el-tree>
        </el-card>
      </el-col>
    </el-row>
    <el-dialog title="分配用户" :visible.sync="centerDialogVisible" center>
      <el-transfer v-model="roleAdmins" :data="allAdmins" :props="{
      key: 'id',
      label: 'account'
    }"></el-transfer>
      <span slot="footer" class="dialog-footer">
        <el-button @click="centerDialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="dispatchAdminDo">确 定</el-button>
      </span>
    </el-dialog>
  ...
  methods: {
    dispatchAdminDo() {
      ...
      net
        .signadminrole({
          params: {
            roleId: this.dispatchId,
            adminIds:this.roleAdmins.join(",")
          }
        })
        .then(data => {
          loading.close();
    ...
    dispatchAdmin(roleId) {
      ...
      const promise1 = net.getAllAdmins().then(data => {
        loading.close();
        if (data.code != "0000") {
          this.$message.error(data.msg);
          return;
      ...
      Promise.all([promise1, promise2])
        .then(() => {
          loading.close();
          if (this.allAdmins && this.roleAdmins) {
            this.centerDialogVisible = true;
            this.dispatchId = roleId;
          } else {
            return;
          }
        })
